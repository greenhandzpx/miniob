...
public:
   ...
   * 扫描指定范围的数据
   */
  IndexScanner *create_scanner(const std::vector<const char *> &left_key, const std::vector<int> &left_len, bool left_inclusive,
			       const std::vector<const char *> &right_key, const std::vector<int> &right_len, bool right_inclusive) override;
...
public:
  ...
  RC open(const std::vector<const char *> &left_key, const std::vector<int> &left_len, bool left_inclusive,
